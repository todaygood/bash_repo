#!/usr/bin/env python
"""
Purpose: export all dashboards from grafana server,version 5.1
...
def export_dashboard(dash,gp):
    ...
    #url = gp.url+"/api/dashboards/uid/%s"%uid
    url = gp.url+"/api/dashboards/%s"%uri
    print("uid=%s,url is:"%uid, url)
    ...
    response = urllib2.urlopen(req)
    body = response.read()
    print("uid=%s,response is:"%uid, body) 

    body_dict = json.loads(body)

    with open(file_name,'w') as outfile:
        json.dump(body_dict,outfile)
